...
public class AutogenProcessor extends AbstractProcessor {
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		...
		try {
			...
			// GENERAL //
			Set<TypeElement> plugins = ElementFilter.typesIn(roundEnv.getElementsAnnotatedWith(PluginInfo.class));
			for (TypeElement element : plugins) { // should only be one, but we have no good/consistent way of checking / won't cause problems
				Iterator<? extends TypeMirror> interfaces = element.getInterfaces().iterator();
			...
			// COMMANDS //
			Set<TypeElement> commands = ElementFilter.typesIn(roundEnv.getElementsAnnotatedWith(CommandInfo.class));
			if (!commands.isEmpty()) {
				...
				elements:
				for (TypeElement element : commands) {
					Iterator<? extends TypeMirror> interfaces = element.getInterfaces().iterator();
					...
					while (true) {
						if (!interfaces.hasNext()) {
							...
							continue elements; //go to next element
						}
						if (typeUtil.isSameType(interfaces.next(), elementUtil.getTypeElement("org.bukkit.command.CommandExecutor").asType()))
							break; //it implements CommandExecutor
					}
					
					Iterator<ExecutableElement> constructors = ElementFilter.constructorsIn(element.getEnclosedElements()).iterator();
					while (true) {
						if (!constructors.hasNext()) {
							msg.printMessage(Diagnostic.Kind.ERROR, "Any class using the @CommandInfo annotation must have a parameterless constructor", element);
							continue elements; //go to next element
						}
						if (constructors.next().getParameters().isEmpty())
							break; //it has empty constructor
					}
					...
					autogenRoot.get(info.command()).putValue("classpath", element.getQualifiedName().toString());
				}
			}
			
			// PERMISSIONS //
			Set<TypeElement> missions = ElementFilter.typesIn(roundEnv.getElementsAnnotatedWith(PermissionInfo.class));
			if (!missions.isEmpty()) {
				if (!root.containsKey("permissions"))
					root.putNode("permissions");
				YamlNode permits = root.get("permissions");
				
				for (TypeElement element : missions) {
